...
describe("LocalCheckpointUnitOfWork", () => {
  it("publishes one aggregate only after explicit commit", async () => {
    ...
    await expect(new LocalSessionArchiveRepository(root).get(archive.identity)).resolves.toBeUndefined();
    await uow.commit();
  ...
  it("refuses to publish an aggregate that references a missing object", async () => {
    ...
    await expect(new LocalSessionArchiveRepository(root).get({ repositoryId: "repo-id", sessionId: "session-id" })).resolves.toBeUndefined();
  });
  ...
  it("rejects stale concurrent aggregate revisions", async () => {
    ...
    const firstUow = new LocalCheckpointUnitOfWork(new LocalSessionArchiveRepository(root), objects);
    const staleUow = new LocalCheckpointUnitOfWork(new LocalSessionArchiveRepository(root), objects);
